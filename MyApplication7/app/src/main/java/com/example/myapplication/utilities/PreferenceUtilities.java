 ...
 */
    ...
    private static void saveEtablissementInffo(final Context context, int n) {
         DatabaseReference etablissementRef = null;
         if (n == 3 ){
             etablissementRef =  rootRef.child("Hopitals");
         }else {
             etablissementRef =  rootRef.child("laboratoir");
         }
    ...
    private static void saveDoctorInfo(final Context context) {
        if (FirebaseAuth.getInstance().getCurrentUser() != null) {
            rootRef.child("Doctor").child(String.valueOf(FirebaseAuth.getInstance().getCurrentUser().getUid())).addListenerForSingleValueEvent(new ValueEventListener() {
                @Override
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
    ...
    private static void savePharmaInfo(final Context context) {
        if (FirebaseAuth.getInstance().getCurrentUser() != null) {
